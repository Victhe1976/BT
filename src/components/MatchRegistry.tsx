...
const MatchRegistry: React.FC<MatchRegistryProps> = ({ players, matches, addMatches }) => {
  ...
  const shareDayResults = () => {
    ...
    lastFinalizedMatches.forEach((match, index) => {
        const teamAPlayers = match.teamA.players.map(pId => playerMap.get(pId)?.name).join(' & ');
        const teamBPlayers = match.teamB.players.map(pId => playerMap.get(pId)?.name).join(' & ');
  ...
  const handleSuggestTeams = async () => {
    ...
    try {
        ...
        const attendedPlayerRankings = allPlayerRankings.filter(p => attendedPlayerIds.has(p.playerId));
        
        if (attendedPlayerRankings.length < 4) {
            throw new Error("Not enough player data to make a suggestion.");
        }
  ...
  const handleUseSuggestion = (matchupIndex: number) => {
    ...
    if (player1A && player2A && player1B && player2B) {
        ...
        alert("Could not find all players from the suggestion. Please select manually.");
    }
  ...
  return (
    <div className="space-y-8">
                        ...
                        <div>
                            ...
                            {dailyMatches.length > 0 ? dailyMatches.map((match, index) => (
                                ...
                                <div className="flex justify-between items-center text-sm">
                                    ...
                                    <span>{match.teamA.players.map(pId => playerMap.get(pId)?.name).join(' & ')}</span>
                                    </div>
                                ...
                                <div className="flex justify-between items-center text-sm">
                                    ...
                                    <span>{match.teamB.players.map(pId => playerMap.get(pId)?.name).join(' & ')}</span>
                                    </div>
      ...
      {isSuggestionModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-lg shadow-xl p-8 w-full max-w-2xl">
                            ...
                            <div key={index} className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                                ...
                                <div className="flex flex-col sm:flex-row items-center justify-around text-center">
                                    <div className="flex items-center gap-2 p-2">
                                        <div className="flex flex-col items-center w-20"><img src={playerNameMap.get(matchup.teamA.player1.toLowerCase())?.avatar} className="w-12 h-12 rounded-full object-cover" alt={matchup.teamA.player1} /><span>{matchup.teamA.player1}</span></div>
                                        <span className="text-xl">+</span>
                                        <div className="flex flex-col items-center w-20"><img src={playerNameMap.get(matchup.teamA.player2.toLowerCase())?.avatar} className="w-12 h-12 rounded-full object-cover" alt={matchup.teamA.player2} /><span>{matchup.teamA.player2}</span></div>
                                    </div>
                                    <span className="font-bold text-cyan-400 text-2xl my-2 sm:my-0">VS</span>
                                    <div className="flex items-center gap-2 p-2">
                                        <div className="flex flex-col items-center w-20"><img src={playerNameMap.get(matchup.teamB.player1.toLowerCase())?.avatar} className="w-12 h-12 rounded-full object-cover" alt={matchup.teamB.player1} /><span>{matchup.teamB.player1}</span></div>
                                        <span className="text-xl">+</span>
                                        <div className="flex flex-col items-center w-20"><img src={playerNameMap.get(matchup.teamB.player2.toLowerCase())?.avatar} className="w-12 h-12 rounded-full object-cover" alt={matchup.teamB.player2} /><span>{matchup.teamB.player2}</span></div>
                                    </div>
                                </div>
