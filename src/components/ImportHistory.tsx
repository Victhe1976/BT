      ...
      try {
        ...
        json.forEach((row, i) => {
            ...
            importedMatches.push({
              ...
              teamA: { players: [playerMap.get(row['Jogador 1'].trim().toLowerCase())!.id, playerMap.get(row['Jogador 2'].trim().toLowerCase())!.id], score: parseInt(row['Dupla A'], 10) },
              teamB: { players: [playerMap.get(row['Jogador 3'].trim().toLowerCase())!.id, playerMap.get(row['Jogador 4'].trim().toLowerCase())!.id], score: parseInt(row['Dupla B'], 10) },
            });
        });
